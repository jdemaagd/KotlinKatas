...
By this logic, we say a sequence of brackets is balanced if the following conditions are met:
    ...
    - Subset of brackets enclosed within confines of a matched pair of brackets is also a matched pair of brackets.
...
// Space Complexity: O(n), worst case where all characters are opening brackets
fun isBalanced(
    s: String
): String {
    ...
    for (c in s) {
        when (c) {
            '(', '{', '[' -> stack.add(c)
            ')' -> if (stack.isEmpty() || stack.removeAt(stack.lastIndex) != '(') return "NO"
            '}' -> if (stack.isEmpty() || stack.removeAt(stack.lastIndex) != '{') return "NO"
            ']' -> if (stack.isEmpty() || stack.removeAt(stack.lastIndex) != '[') return "NO"
        }
    }
